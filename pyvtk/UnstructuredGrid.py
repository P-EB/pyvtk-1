...
class UnstructuredGrid(DataSet.DataSet):
    ...
    def __init__(self, points, vertex=[], poly_vertex=[], line=[],
                 poly_line=[], triangle=[], triangle_strip=[],
                 polygon=[], pixel=[], quad=[], tetra=[], voxel=[],
...
def unstructured_grid_fromfile(f, self):
    ...
    while len(tps) < n:
        tps += list(map(int, common._getline(f).split()))
    assert len(tps) == n
    dictionary = {}
